...
<template>
    <Head>
        <Title>{{ categoryInfo.name ? categoryInfo.name : "" }} - Clothing Store Online</Title>
    ...
    <div class="page-container">
        <aside :class="{ active: mobileMenuActive }">
            <div class="filter-container">
                ...
                <ul v-if="categoryInfo.department" class="filter-list">
                    <li v-for="subcategory in categoryInfo.subcategories" :key="subcategory.id" class="filter-list__item">
                        <NuxtLink :to="{name: 'category-id', params: {id: subcategory.slug}}" >{{ subcategory.name }}</NuxtLink>                     
                    </li>
                </ul>
                ...
                if (item.slug == this.$route.params.id) {
                    ...
                    if(item.subcategories){
                        item.subcategories.map(item2 => {
                            if(item2.slug == this.$route.params.id) {
                                this.categoryInfo = item2
                            }
                        })
                    }
                }
            })
            // this.categoryInfo = infoJson.categories.filter(item => item.slug == this.$route.params.id).pop();
        },
